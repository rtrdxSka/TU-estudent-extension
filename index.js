...
const darkMode = () => {
  ...
  const textElements = document.querySelectorAll('p, h2, h3, h4, h5, h6, span, div, table, th');
...
`;
  ...
  const scaryButton = document.querySelector('input[type="button"][value="Изход"]');
  ...
  if (scaryButton) {
    ...
    scaryButton.addEventListener('mousedown', () => {
      scaryButton.style.backgroundColor = '#8e1414'; /* Even darker red when pressed */
      scaryButton.style.transform = 'scale(1)';
    });

    scaryButton.addEventListener('mouseup', () => {
      scaryButton.style.backgroundColor = '#a11a1a'; /* Restore hover color */
    });
  }
